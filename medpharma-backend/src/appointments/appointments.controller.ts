...
export class AppointmentsController {
  ...
  createAppointment(@Body() body: { patientName: string; doctorId: string }) {
    // Remove unused 'error' variable - just remove the try/catch if not needed
  ...
  updateAppointmentStatus(
    ...
    // Fix unsafe argument - properly type the status
    ...
    const success = this.appointmentsService.updateAppointmentStatus(
      id,
      body.status as Appointment['status'] // Fix: properly cast to valid status type
    );
